...
2. **Dozen different frameworks**
   ...
   This leads AWS and GCP to pick and choose which ones they support.
...
1. **Break down the problem**
   ...
   Deploying a frontend is a combination of generating the _build output_ through something called an adapter. And then using that to deploy the infrastructure.
   ...
   This is what led us to create the [OpenNext](https://opennext.js.org) project.
...
The downside of our approach is that we **recreate all the infrastructure** (like the CDN distribution) for each of your sites. This means it can take 15-20 mins to create one of these. So if you have multiple frontends or if you were creating preview environments, your deployments will be slow.
...
Taking a step back here for a second. A dedicated service like Netlify or Vercel is still likely to give you a better out-of-the-box experience.
...
- The [`Router`](/docs/component/aws/router/) component docs.
- A guide on [**configuring a router**](/docs/configure-a-router).
- Setting the [`router`](/docs/component/aws/nextjs/#router) prop in your frontend.
- Or adding multiple [`regions`](/docs/component/aws/nextjs/#regions) to your frontend.
