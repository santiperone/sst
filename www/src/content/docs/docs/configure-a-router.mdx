...
Instead of adding custom domains to each component, let's add a `Router` to our app with the domain.
...
```ts title="sst.config.ts"
const domain = (() => {
  if ($app.stage === "production") return "example.com";
  if ($app.stage === "dev") return "dev.example.com";
  return;
})();
```

Now update our Router to use the domain.
...
For our PR stages we want to share the same CloudFront distribution from dev, instead of creating a new one per stage.
...
const domain = (() => {
  ...
  if ($app.stage === "dev") return "dev.example.com";
-  return;
+  return `${$app.stage}.dev.example.com`;
})();
```
